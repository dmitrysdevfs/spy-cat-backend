...
class SpyCatViewSet(viewsets.ModelViewSet):
    ...
    def destroy(self, request, *args, **kwargs):
        # A cat can be deleted normally unless we want to enforce other logic
...
class MissionViewSet(viewsets.ModelViewSet):
    ...
    def assign_cat(self, request, pk=None):
        ...
        if not cat_id:
            return response.Response(
                {"error": "Cat ID is required."}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        # Check if cat already has an active (not completed) mission
        if Mission.objects.filter(cat=cat).exclude(status="completed").exists():
            return response.Response(
                {"error": "Cat already has an active mission."},
                status=status.HTTP_400_BAD_REQUEST,
            )
    ...
    @decorators.action(
        detail=True, methods=["patch"], url_path="targets/(?P<target_id>[^/.]+)"
    )
    def update_target(self, request, pk=None, target_id=None):
        ...
        if notes:
            if target.status == "completed" or mission.status == "completed":
                return response.Response(
                    {
                        "error": "Notes cannot be updated because the target or mission is completed."
                    },
                    status=status.HTTP_400_BAD_REQUEST,
                )
            target.notes = notes
        ...
        # Check if all targets are completed to auto-complete mission
