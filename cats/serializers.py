...
class SpyCatSerializer(serializers.ModelSerializer):
    ...
    def validate_breed(self, value):
        if not validate_breed(value):
            raise serializers.ValidationError(
                f"'{value}' is not a valid cat breed according to TheCatAPI."
            )
        return value
    ...
    def update(self, instance, validated_data):
        ...
        if len(validated_data) > 1 or (
            len(validated_data) == 1 and "salary" not in validated_data
        ):
            raise serializers.ValidationError(
                "Only 'salary' can be updated for a spy cat."
            )
